...
import com.tanlifei.exemple.dialog.extra.ExempleDialogIOSTaoBaoDialog;
...
public class ExempleDialogDialogHomeActivity extends BaseActionBarActivity implements ExpandableListView.OnChildClickListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mMenuItems.add(new DialogMenuItem("删除", R.mipmap.exemple_dialog_winstyle_delete));
        mMenuItems.add(new DialogMenuItem("歌手", R.mipmap.exemple_dialog_winstyle_artist));
        mMenuItems.add(new DialogMenuItem("专辑", R.mipmap.exemple_dialog_winstyle_album));



        View decorView = getWindow().getDecorView();
        mElv = ViewFindUtils.find(decorView, R.id.elv);
        ExempleDialogHomeAdapter adapter = new ExempleDialogHomeAdapter(mContext);
    ...
    public boolean onChildClick(ExpandableListView parent, View v, int groupPosition, int childPosition, long id) {
        ...
        if (groupPosition == 1) {
            if (childPosition == 0) {
                final ExempleDialogCustomBaseDialog dialog = new ExempleDialogCustomBaseDialog(mContext);
                dialog.show();
                dialog.setCanceledOnTouchOutside(false);
            } else if (childPosition == 1) {
                final ExempleDialogShareBottomDialog dialog = new ExempleDialogShareBottomDialog(mContext, mElv);
                dialog.show();//
                // .show(0, 100);
            } else if (childPosition == 2) {
                final ExempleDialogShareTopDialog dialog = new ExempleDialogShareTopDialog(mContext);
                dialog.show();
                // .show(0, 100);
            }
        }
        ...
        if (groupPosition == 3) {
            if (childPosition == 0) {
                final ExempleDialogIOSTaoBaoDialog dialog = new ExempleDialogIOSTaoBaoDialog(mContext, (View) mElv.getParent());
                dialog.show();
                // .show(0, 100);
            }
        }
    ...
    private void NormalListDialogAdapter() {
        final NormalListDialog dialog = new NormalListDialog(mContext, new ExempleDialogTestAdapter(mContext, mMenuItems));
        dialog.title("请选择")//
                .show();
        dialog.setOnOperItemClickL(new OnOperItemClickL() {
            @Override
